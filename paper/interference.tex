...
%
% - Uses(bb)
%       returns all uses withtin basic block bb 
% - Users(R, i)
  ...
  \For{$p \in \FPersistentInstructions{R}$}{
    \For{$bb \in \FBasicBlocks{R} \mid \neg \FConnected(\FBasicBlock(p), bb)$}{
      ...
      \If{$\FIndex(bb) > \FIndex(\FBasicBlock(p))$}{
        ...
        \Comment{Case 1.1}
        \For{$i \in \FUsers(R, p)$}{
          \Assert{$\FIndex(i) < \FIndex(bb)$}\;
          \Comment{Else bug in \AMi{} tranformation?}
        }

        \Comment{Case 1.2}
        \For{$i \in \FUses(bb) \mid \FIndex(i) < \FIndex(p)$}{
          % TODO: Unless u already contains that slot index
          $E \gets E \cup \FEdge(p,i)$\;
          \Comment{LLVM: Add Segment(p) to LIS(u)}
        }
      }
