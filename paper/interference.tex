...
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.5\textwidth]{cases/cfg}
  \caption{\texttt{def-list} \brackets{\leftarrow\ \texttt{use-list}}}
\end{figure}

\begin{figure}[h!]
  \centering
  \subfloat[CASE 1\\BB0 < \textcolor{myred}{BB1} < \textcolor{myblue}{BB2} < BB3]{\includegraphics[width=0.35\textwidth]{cases/lin1}} \hfill
  \subfloat[CASE 2\\BB0 < \textcolor{myblue}{BB2} < \textcolor{myred}{BB1} < BB3]{\includegraphics[width=0.35\textwidth]{cases/lin2}}
...
% In a control-flow graph, a join point is a location where multiple control paths in the program converge. There can be multiple join points between two disconnected basic blocks.
...
\todo{Does it generalize to arbitrary control-flow graphs?}
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.5\textwidth]{cases/case_1_2}
  \caption{Case 1.2}
\end{figure}
